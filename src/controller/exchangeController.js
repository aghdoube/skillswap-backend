...
export const requestExchange = async (req, res, next) => {
    try {
        ...
        if (!provider || !skill) {
            return res.status(400).json({ message: "Provider and skill are required" });
        }
        ...
        const providerSkill = await Skill.findOne({ _id: skill, user: provider });
        if (!providerSkill) {
            return res.status(404).json({ message: "Skill not found for this provider" });
        }
...
export const getUserExchanges = async (req, res, next) => {
    try {
        ...
        let query = { $or: [{ requester: req.user._id }, { provider: req.user._id }] };
...
export const getExchangeMessages = async (req, res, next) => {
    try {
        ...
        if (!exchange.requester.equals(req.user._id) && !exchange.provider.equals(req.user._id)) {
        ...
        res.json({
            ...
            message: "Fetch messages using your messaging service with the conversationId"
        });
    } catch (error) {
        next(error);
    }
